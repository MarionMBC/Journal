...
const NoteAppContent = ({id, noteTitle, noteBody, noteUrl, handleUpdate}) => {
    ...
    return (<div className={'flex flex-col text-start text-white p-5'}>
        ...
        <AiFillDelete onClick={handleDelete}  className={'cursor-pointer sm:hidden w-8 h-8 right-0 me-5 absolute top-[90px] text-yellow-400 hover:text-red-500 cursor-pointer hover:scale-105 transition-all easy-in-out'} />
            <input
                name={'title'}
                value={title}
